...
do 
	...
	docker run -d --rm -p ${PORT}:22 --name server_${CLIENT} --network=test-net my-ansi-client
done
...
do 
	PORT=$(( ${BASE_PORT} + ${CLIENT} ))
	docker run -d --rm -p ${PORT}:22 --name app_server_${CLIENT} --network=test-net my-ansi-client
done
...
do 
	PORT=$(( ${BASE_PORT} + ${CLIENT} ))
	docker run -d --rm -p ${PORT}:22 --name app2_server_${CLIENT} --network=test-net my-ansi-client
done
